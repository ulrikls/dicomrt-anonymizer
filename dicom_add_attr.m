function attr_str = dicom_add_attr(attr_str, group, element, dictionary, varargin)
...
%   $Revision: 1.1.6.7.2.1 $  $Date: 2011/07/18 00:38:40 $
...
for idx = 1:numel(numData)
    ...
    str_append = ['\', sprintf(fmtString, numData(idx))];
    str_offset = length(str_append);
    
    if (str_pos + str_offset > str_free)
        str_free = str_free + 1000;
        strData((str_pos + 1):str_free) = ' ';
    end
    
    strData((str_pos + 1):str_offset) = str_append;
    str_pos = str_pos + str_offset;
  
end

% Remove the leading '\' from the first iteration.
strData = strData(2:str_pos);
